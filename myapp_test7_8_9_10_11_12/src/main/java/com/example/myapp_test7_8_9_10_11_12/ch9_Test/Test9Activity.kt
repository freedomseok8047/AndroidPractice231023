package com.example.myapp_test7_8_9_10_11_12.ch9_Test
...
import com.example.myapp_test7_8_9_10_11_12.ch7_Test.TestActivity
import com.example.myapp_test7_8_9_10_11_12.databinding.ActivityTest9Binding
import javax.net.ssl.CertPathTrustManagerParameters

class Test9Activity : AppCompatActivity() {
    // 바인딩 이름 구칙은 , 레이아웃 이름을 따라 갑니다. (자동생성)
...
//        setContentView(R.layout.activity_test9)
        ...
        // 소스코드로, 정적 자원 사용하기 . 문자열
        activityTest9Binding.testView2.text = getString(R.string.app_intro)


        // 해당 라이브러리 기능을 확인 하는 부분 보다
        // sdk 버전에 달라서, 라이브러리 패키지 명 등 다른 부분에 집중
        // 사용하는 라이브러리의 sdk 버젖의 지원 여부 확인.
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.R){
            val windowMetrics: WindowMetrics = windowManager.currentWindowMetrics
            Log.d("ljs", "width: ${windowMetrics.bounds.width()}, height: ${windowMetrics.bounds.height()}")
        }else{
        ...
        if (status == PackageManager.PERMISSION_GRANTED){
            ...
            Log.d("ljs", "권한이 승인됨 : ${status}")
        }else
        {
            Toast.makeText(this@Test9Activity, "위치 권한 승인 안 됨", Toast.LENGTH_SHORT).show()
            Log.d("ljs", "권한이 승인 안 됨 : ${status}")
        }

        // 후처리, 인텐트 기본적인 사용법 단순 페이지 이동만 사용했음,
        // 인텐트? 시스템에 메세지를 전달자
        // 예) 페이지 이동같은 경우, 시스템에 요청해서 , 이동했음
        // 예) 특정 앱에 접근을 해서, 데이터를 가져오는 작업 (=후처리)
        // 설정 1)
        val requestPermissionLauncher = registerForActivityResult(
            // 이 부분이 시스템에서 정해둔 함수들이 있음. 현재, 허가를 확인 하는 용도.
            // 나중에, 이미지등 데이터에 접근해서, 해당 데이터를 가지고 오는 용도로 사용할 예정
            ActivityResultContracts.RequestPermission()){
            isGranted ->
            if(isGranted){
                Log.d("ljs", "권한이 승됨, call back 후처리 요청.")
            }else{
                Log.d("ljs", "권한이 승인 안 됨, call back 후처리 요청.")
            }
        }
        // 이용 -> 호출, 위에 설정으로
        ...
        activityTest9Binding.testToastBtn?.setOnClickListener{
            ...
            val toast = Toast.makeText(this@Test9Activity,"후처리 확인중", Toast.LENGTH_SHORT)

            toast.addCallback(
                object : Toast.Callback(){
                    override fun onToastHidden() {
                        super.onToastHidden()
                        Log.d("ljs","토스트 후처리 작업: 사라질 경우 ")
                        val intent = Intent(this@Test9Activity, TestActivity::class.java)
                        startActivity(intent)
                    }
        ...
        activityTest9Binding.dateBtn?.setOnClickListener{
            DatePickerDialog(this@Test9Activity, object : DatePickerDialog.OnDateSetListener{
                override fun onDateSet(view: DatePicker?, year: Int, month: Int, dayOfMonth: Int) {
                    Log.d("ljs","년도: ${year}년 , 월: ${month+1}월 , 일: ${dayOfMonth}일")
                    Toast.makeText(this@Test9Activity,"년도: ${year}년 , 월: ${month+1}월 , 일: ${dayOfMonth}일",
        ...
        activityTest9Binding.timeBtn?.setOnClickListener{
            TimePickerDialog(this@Test9Activity,object: TimePickerDialog.OnTimeSetListener{
                override fun onTimeSet(view: TimePicker?, hourOfDay: Int, minute: Int) {
                    Log.d("ljs","${hourOfDay}시 ${minute}분")
        ...
        // 목록 요소 선택1
        val items = arrayOf("사과","바나나","수박", "파인애플")
        ...
        activityTest9Binding.customDialogBtn2?.setOnClickListener{
            AlertDialog.Builder(this@Test9Activity).run {
                ...
                // setMessage("테스트 할까요?")
                // 추가 사항
                val objectListner = object : DialogInterface.OnClickListener{
                    override fun onClick(dialogInterface: DialogInterface?,which : Int){
                        Log.d("ljs","선택한 과일: ${items[which]}")
                    }
                }
                setItems(items,objectListner)
                setPositiveButton("수락",null)
                setNegativeButton("취소", null)
                setNeutralButton("더보기",null)
                show()
        ...
        AlertDialog.Builder(this@Test9Activity).run {
            ...
            // 추가 사항
            //체크박스용 클릭 리스너,
            val objectListner = object :DialogInterface.OnMultiChoiceClickListener{
                override fun onClick(dialog: DialogInterface?, which: Int, isChecked: Boolean) {
                    Log.d("ljs","${items[which]}이 ${if(isChecked)"선택됨" else "선택 해제 됨"}")
                }
            }
            // 목록요소1
            //setItems(items,objectListner)
            ...
            // 목록 요소 2, 체크박스
            setMultiChoiceItems(items, booleanArrayOf(true,true,false,false),objectListner)
